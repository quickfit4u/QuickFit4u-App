...
You should see: `FitPass India backend running at http://localhost:3000`
...
2. **Wiring the FitPass India app screens** to these endpoints instead of mock data.
...
1. **Wire the FitPass India app screens** to these endpoints (Home, Find a Gym,
...
- Gyms now have `latitude`/`longitude` columns (existing databases are
  migrated automatically on server start — no need to delete `data/fitpassindia.db`).
