...
// Get my gym profile (null if not created yet)
...
// Create my gym profile (one gym per owner account for now)
...
// ---------- Bank / payout details ----------
// Where the platform sends this owner's share of bookings. No live payout
// API is wired up (see lib notes) — this is just where it's registered so
// an admin can see it and settle manually. Not included in the public gym
// payload (serializeGym) — only the owner themself and the admin dashboard
// can see it.
...
// body: { accountHolder, accountNumber, ifsc, upiId } — a UPI ID alone is
// enough (no bank account required), same as most Indian payout forms; at
// least one of the two is required so there's *some* way to pay the owner.
...
// Add an hourly slot for a given date
// body: { date: 'YYYY-MM-DD', hourLabel: '6 AM', capacity: 3 }
...
// List my slots for a given date (defaults to today), with how many are booked
...
router.get('/mine/bookings/today', requireAuth, requireRole('owner'), (req, res) => {
  ...
  res.json({
    ...
    // Frontend can check bookings.length === 0 and show "No bookings for today yet."
  });
});
...
router.get('/mine/dashboard', requireAuth, requireRole('owner'), (req, res) => {
  ...
  const dayIndex = (todayDate.getUTCDay() + 6) % 7; // 0 = Monday
  ...
  // Revenue = paid amount (in rupees) for bookings whose slot falls in the
  // given date range, regardless of whether the visit has happened yet —
  // the payment itself is what counts as revenue.
  ...
  // A no-show: a confirmed booking whose slot start time has already
  // passed, and the member never checked in.
...
// A gym stays hidden from public search/detail (see the public routes below)
// until its owner signs this agreement. On signing, we generate a QR code
// for the gym, save it to the profile, and email it to the owner.
router.post('/mine/agreement', requireAuth, requireRole('owner'), async (req, res) => {
  ...
  // Encode a simple, verifiable reference to this gym — the front-desk /
  // any future scanner just needs to resolve this back to a gym id.
  const qrPayload = JSON.stringify({ app: 'FitPass India', type: 'gym', gymId: gym.id });
  let qrDataUrl;
  try {
  ...
  try {
    ...
    // Don't fail the whole request if the email fails to send — the QR is
    // already saved in the app either way. Just log it server-side.
...
// ---------- Member-facing: browse gyms (public, no auth needed) ----------

// True if this gym has a slot today that has already started, hasn't ended
// yet (assumes 1-hour slots, matching how the rest of the app treats them),
// and still has room. Used by the `openNow` filter.
...
// A rough, live "how busy is it right now" signal for members — based on
// how full today's currently-active slot is (or, if there's no slot running
// right now, how full today's slots are on average). Returns null when
// there's no data yet to judge from.
...
// GET /api/gyms?city=Bengaluru&search=delhi&minPrice=100&maxPrice=500
//              &minRating=4&facilities=AC,Parking&ac=true&personalTrainer=true
//              &parking=true&shower=true&femaleFriendly=true&openNow=true
//              &lat=12.9&lng=77.6&maxDistanceKm=10&sortBy=distance|price|rating
// `search` matches gym name, area, city, or tags (case-insensitive, partial match).
// Only shows gyms whose owner has signed the partnership agreement.
router.get('/', (req, res) => {
  const {
  ...
  // Rating depends on the reviews aggregate computed in withRatings, so it
  // has to be filtered in JS rather than in the SQL above.
  if (minRating !== undefined && minRating !== '') {
    const min = Number(minRating);
    results = results.filter((g) => g.rating != null && g.rating >= min);
  }

  // Facility filters — a gym must have ALL of the requested tags.
  ...
  // Distance — only meaningful once we have the member's current position.
...
// GET /api/gyms/:id?date=YYYY-MM-DD
...
// POST /api/gyms/:id/reviews — leave a review (members only)
// Only allowed once the member has actually checked in at this gym at least
// once — otherwise anyone could post a review without ever visiting, which
// is exactly the fake-review problem this guards against. One review per
// member per gym: a second submission updates the existing one rather than
