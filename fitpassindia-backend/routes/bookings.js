...
// Frees up slots held by abandoned checkouts (app closed mid-payment, user
// backed out without hitting cancel, etc). Cheap enough to run inline on
// every create-order call — no cron needed for this scale.
...
async function generateAndSendBookingQr(booking, gym, slot) {
  // Contains the booking ID, gym ID, an expiry (end of the slot's day —
  // the QR stops working the day after the booking), and a signed token so
  // a tampered or copied QR can be rejected without a database round trip.
...
// ======================================================================
// MEMBER: step 1 — reserve the slot and create a Razorpay order to pay for
// it. The booking is created as 'pending_payment' (not shown in "My
// Bookings") and only becomes a real, owner-visible 'confirmed' booking once
// /verify-payment succeeds. Reserves the slot immediately so two people
// can't pay for the same hour at once.
// body: { slotId, note }
// ======================================================================
router.post('/create-order', requireAuth, requireRole('member'), async (req, res) => {
  ...
  try {
    ...
    // Order creation failed (e.g. Razorpay not configured / network hiccup)
    // — release the slot we just reserved so it isn't stuck held forever.
...
// ======================================================================
// MEMBER: step 2 — verify the payment Razorpay's checkout returned. On
// success the booking becomes 'confirmed' right here — no separate owner
// accept/reject for a normal paid booking. Generates the booking QR, emails
// it, and lets the gym owner know (FYI, not an approval request).
...
// ======================================================================
// MEMBER: called if the user backs out of the Razorpay checkout instead of
// paying — releases the held slot right away instead of waiting for the
// stale-hold cleanup.
// ======================================================================
router.post('/:id/cancel-pending-payment', requireAuth, requireRole('member'), (req, res) => {
  ...
  if (booking.status !== 'pending_payment') {
    return res.json({ ok: true }); // already resolved one way or another — nothing to do
  }
...
// ======================================================================
// MEMBER: self check-in by scanning the GYM's QR code (used when there's
// no staff around to scan the member's own booking QR).
// body: { gymId }  — decoded client-side from the scanned gym QR
// ======================================================================
...
// ======================================================================
// MEMBER: self check-in by typing their own booking code instead of
// scanning the gym's QR — fallback for when camera scanning misbehaves.
// body: { bookingCode }
// ======================================================================
...
// ======================================================================
// OWNER: check a member in by scanning THEIR booking QR
// body: { bookingId, gymId, expiresAt, token } — decoded client-side from
// the scanned booking QR (see generateAndSendBookingQr above for how it's
// generated).
// ======================================================================
router.post('/owner-checkin', requireAuth, requireRole('owner'), (req, res) => {
  ...
  // Older QRs (generated before this field set existed) won't carry a
  // token — only enforce the signature/expiry check when one is present.
...
// ======================================================================
// OWNER: check a member in by typing their booking code instead of
// scanning — the reliable fallback when camera scanning misbehaves (a
// known Expo Go limitation, not something the app controls). Same
// check-in logic as /owner-checkin, just looked up by code instead of QR.
// body: { bookingCode }
// ======================================================================
...
// ======================================================================
// MEMBER: request to reschedule an already-confirmed (paid) booking to a
// different slot at the same gym — e.g. running late, or a time conflict.
// This does NOT touch the original booking yet: it just reserves the new
// slot and records the request. The member's original slot stays booked
// and the booking stays 'confirmed' the whole time, so if the owner
// declines, nothing is lost and no refund is needed.
// body: { newSlotId, note }
// ======================================================================
router.put('/:id/reschedule', requireAuth, requireRole('member'), (req, res) => {
  ...
  const requestReschedule = db.transaction(() => {
    ...
    // Hold the requested slot so it can't be taken while the owner decides
    // — the original slot is untouched and stays reserved either way.
...
// ======================================================================
// OWNER: accept a reschedule request — swaps the booking onto the new
// slot and frees the old one. The booking stays 'confirmed' throughout
// (same booking id, same QR — nothing to re-send).
// ======================================================================
...
// ======================================================================
// OWNER: decline a reschedule request — frees the held new slot and
// leaves the original booking exactly as it was. No refund needed since
// the original slot/payment was never touched.
// ======================================================================
...
// ======================================================================
// OWNER: list pending reschedule requests for my gym (newest first)
// ======================================================================
...
// ======================================================================
// MEMBER: my booking history (includes pending / confirmed / rejected)
// ======================================================================
