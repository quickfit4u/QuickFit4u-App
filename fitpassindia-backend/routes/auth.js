...
// Accepts 'YYYY-MM-DD' and rejects future dates (a date of birth can't be
// in the future).
...
// ======================================================================
// SIGNUP — step 1: send a code to the email (no account created yet)
// body: { email, name, role: 'member' | 'owner', referredBy? }
// ======================================================================
...
// ======================================================================
// SIGNUP — step 2: verify the code AND set a password, then create the account
...
router.post('/login', async (req, res) => {
  ...
  if (!user.password_hash) {
    // Legacy account created before passwords existed — needs to set one via Forgot Password.
...
router.post('/reset-password', async (req, res) => {
  ...
  // Log them straight in after a successful reset — one less step.
  const token = signToken(user);
  res.json({ ok: true, token, user: publicUser(user) });
});
...
// ======================================================================
// UPDATE PROFILE — name, phone, gender, address, avatar, date of birth,
// emergency contact, fitness goal, blood group. Email/role/password are not
// editable here (email is reset via forgot-password; role is fixed at signup).
// body: { name?, phone?, gender?, address?, avatarUrl?, dateOfBirth?,
//         emergencyContact?, fitnessGoal?, bloodGroup? }
// ======================================================================
...
// ======================================================================
// DELETE ACCOUNT — removes the user and everything tied to them.
// Members: their bookings and reviews are removed.
// Owners: their gym, that gym's slots, and its bookings/reviews are removed too.
// ======================================================================
...
// ======================================================================
// PUT /api/auth/push-token — register (or clear) this device's Expo push
// token, so notify() can reach this user even when the app isn't open.
// Called on login/app-start once notification permission is granted, and
// with pushToken: null on logout so a shared/reset device stops getting
