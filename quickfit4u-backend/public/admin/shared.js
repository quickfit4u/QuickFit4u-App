...
function renderShell(activePage) {
  ...
  document.body.insertAdjacentHTML(
    ...
    <div class="layout">
      <aside class="sidebar">
        <div class="brand">QuickFit4u <span>Admin</span></div>
        ${NAV_ITEMS.map(
  ...
  document.getElementById('adminName').textContent = localStorage.getItem('quickfit4u_admin_name') || '';
  document.getElementById('sidebarLogout').addEventListener('click', (e) => {
    e.preventDefault();
    localStorage.removeItem('quickfit4u_admin_token');
    localStorage.removeItem('quickfit4u_admin_name');
    window.location.href = 'login.html';
  });
...
function setPageTitle(title) {
  ...
  document.title = 'QuickFit4u Admin — ' + title;
}
