...
export default function App() {
  ...
  // Tracks which screen to return to after the member scanner closes — it can be
  // opened either from a specific booking's detail page or straight from the
  // home dashboard's "Scan Gym QR" button, so the back target isn't fixed.
  ...
  async function enterAppAs(loggedInUser) {
    ...
    if (loggedInUser.role === 'owner') {
      ...
      // New or not-yet-finished onboarding goes straight into the setup wizard,
      // picking up at whichever step isn't done yet; a fully onboarded owner
      // (agreement signed) lands on the dashboard.
