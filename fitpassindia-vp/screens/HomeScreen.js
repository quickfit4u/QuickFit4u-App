...
function getWeekStrip() {
  ...
  const dayIndex = (today.getDay() + 6) % 7; // make Monday index 0
...
export default function HomeScreen({ user, onOpenGym, onLogout, onNavigate, onAccountDeleted }) {
  ...
  useEffect(() => {
    if (!user) return; // guests have no notifications to fetch
    fetchNotifications()
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: () => {
            // Second, final confirmation before anything is actually deleted.
            Alert.alert(
  ...
  // Debounced live search — searches gym name, area, and city across all gyms
  // (not just the ones near the user), so typing a place like "Delhi" works
  // even if the user isn't currently near Delhi.
  useEffect(() => {
