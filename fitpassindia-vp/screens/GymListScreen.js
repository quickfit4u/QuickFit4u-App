...
export default function GymListScreen({ onBack, onOpenGym }) {
  ...
  const [coords, setCoords] = useState(null); // { lat, lng } — null until granted/fetched
  const [locatingUser, setLocatingUser] = useState(false);

  // Debounce the free-text search so we're not hitting the server on every keystroke.
  useEffect(() => {
  ...
  async function loadGyms({ isRefresh = false } = {}) {
    ...
    // Distance sorting/filtering needs the device's location — fetch it
    // lazily, only once, the first time it's actually needed.
    let loc = coords;
  ...
  async function applyFilters() {
    // If the person picked a distance-related option, make sure we actually
    // have their location before closing the sheet — otherwise silently
    // fetch it now so the very next load has it.
